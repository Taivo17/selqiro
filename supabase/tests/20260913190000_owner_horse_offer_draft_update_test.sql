...
-- Legacy save stays unchanged, but its writes must invalidate a loaded new-editor revision.
do $legacy_writer$
...
begin
  ...
  perform * from public.save_my_horse_offer_draft_v1(p_offer_id=>v_id,p_offer_type=>'co_rider',
    p_title=>'legacy session update',p_price_type=>'contact',p_recurring_fee_period=>'month');
  ...
  perform pg_temp.horse_edit_assert(v_after.edit_revision::bigint=v_before.edit_revision::bigint+1,
    'legacy writer advances new-editor revision (legacy itself is not CAS-protected)');
  perform pg_temp.horse_edit_expect(format(
...
\echo HORSE_DRAFT_UPDATE_CONTRACT_TEST=PASS
rollback;
